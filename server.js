...
app.post("/api/checkout/create-session", async (req, res) => {
  try {
    ...
    for (const item of cartItems) {
      ...
      // Look up by seller_id and title since cart stores constructed product_id
      let productQuery = supabaseAdmin
        .from("ProductTable")
        .select("*, student:seller_id(first_name, last_name)");
      
      // Try to find by actual product_id first (if it's a UUID)
      // Otherwise, use seller_id and title
      if (item.product_id && item.product_id.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)) {
        // It's a UUID, try direct lookup
        productQuery = productQuery.eq("product_id", item.product_id);
      } else if (item.seller_id && item.title) {
        // Use seller_id and title to find the product
        productQuery = productQuery
          .eq("seller_id", item.seller_id)
          .eq("title", item.title);
      } else {
        // Fallback: try the constructed product_id format
        const parts = item.product_id ? item.product_id.split('_') : [];
        if (parts.length >= 2) {
          ...
          return res.status(400).json({ error: `Invalid product identifier: ${item.title}` });
        }
      }
      ...
      if (productError || !product) {
        ...
        console.error("Looking for product with:", { 
          product_id: item.product_id, 
          seller_id: item.seller_id, 
          title: item.title 
        });
...
app.post("/api/checkout/verify-payment", async (req, res) => {
  try {
    ...
    // Get order to calculate totals (needed for purchase creation)
    ...
    // Generate order number from order_id (first 8 characters, uppercase)
    const orderNumber = orderId.substring(0, 8).toUpperCase().replace(/-/g, '');

    // Create a single purchase record for the buyer
    const { data: purchase, error: purchaseError } = await supabaseAdmin
      .from("purchases")
      .insert({
    ...
    if (purchaseError) {
      ...
      // Fetch product and seller details for purchase_items
      const purchaseItemsWithDetails = await Promise.all(orderItems.map(async (item) => {
        // Get product details
        let imageUrl = null;
        let category = null;
        let condition = null;
        ...
        if (item.product_id) {
          const { data: product } = await supabaseAdmin
            .from("ProductTable")
            .select("image_url, category, condition")
        ...
        if (item.seller_id) {
          ...
          if (seller) {
            sellerName = `${seller.first_name || ''} ${seller.last_name || ''}`.trim() || 'Student';
          }
        }
      ...
      const { error: purchaseItemsError } = await supabaseAdmin
        .from("purchase_items")
        .insert(purchaseItemsWithDetails);

      if (purchaseItemsError) {
        console.error("Purchase items creation error:", purchaseItemsError);
      }
    }
